...
st.write("A Dow 30 indexben szereplő vállalatok részvényeinek intézményi tulajdonosi szerkezete betekintést nyújt a piac nagy szereplőinek körébe. Ez a vizualizáció azt mutatja, hogy az egyes Dow 30 cégekben milyen értékben tulajdonosok ezek az intézmények, valamint rávilágít arra, hogy mely befektetők a legnagyobb tulajdonosok az indexbe tartozó különböző vállalatoknál.")
holders = get_holders()
fig = plt.figure()
holders.groupby("Holder").sum("Value")["Value"].sort_values().tail(10).plot(kind="barh")
...
st.write("A Dow 30 index cégeinek szektoronkénti átlagos adósságát bemutató vizualizáció segít megérteni az egyes iparágak pénzügyi struktúráját és kockázati profilját. A különböző szektorok eltérő működési modelljei miatt az adósságszintek jelentősen változhatnak. Az adósságszint elemzése szektoronként nemcsak a vállalatok kockázati profiljának megértésében segít, hanem betekintést nyújt az iparágak tőkeszerkezetébe és a pénzügyi stratégia mögötti dinamikába is.")
fig = plt.figure()
sb.barplot(data=dow30, x="Debt", y="Sector", orient="h", errorbar=None)
plt.xlabel("Adósság (USD)")
plt.ylabel("Szektor")
plt.title("Átlagos adósság szektoronként")
st.write(fig)
